...
object PersistentModel extends Model:
  ...
  private var nextID: Id = idGenerator.nextId()
  ...
  def update(id: Id)(f: Task => Task): Option[Task] =
    idStore.updateWith(id)(opt => opt.map(f)) match
      case Some(t) =>
        saveTasks(Tasks(idStore))
        Some(t)
      case None =>
  ...
  def tasks(tag: Tag): Tasks =
    Tasks(idStore.filter((id: Id, task: Task) => task.tags.contains(tag)))

  def complete(id: Id): Option[Task] =
    update(id)(_.complete)

  def tags: Tags =
    Tags(idStore.values.flatMap(t => t.tags).toList.distinct)
