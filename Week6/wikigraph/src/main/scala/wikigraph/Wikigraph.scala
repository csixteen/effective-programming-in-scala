...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap { linkedIds =>
      WikiResult.traverse(linkedIds.toSeq)(client.nameOfArticle).map(_.toSet)
    }
  ...
  /** Computes the distance between two articles using breadth first search.
    ...
    * ignoring the problematic node. On the other hand, any system failure just
    * ends the algorithm by returning that system failure.
    *
    ...
    def iter(
      ...
      q.dequeueOption match
        ...
        case Some(((d, elem), newQueue)) =>
          if d >= maxDepth then WikiResult.successful(None)
          else
            client
              .linksFrom(elem)
              .flatMap { linkedIds =>
  ...
  def distanceMatrix(
    ...
    WikiResult.traverse(pairs) { (srcTitle, dstTitle) =>
      client.searchId(srcTitle).zip(client.searchId(dstTitle)).flatMap {
        (srcId, dstId) =>
          breadthFirstSearch(srcId, dstId, maxDepth).flatMap { optDist =>
            WikiResult.successful(srcTitle, dstTitle, optDist)
          }
      }
    }
end Wikigraph
