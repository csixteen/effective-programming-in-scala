    ...
    forAll(genAB) { (a: WikiResult[Int], b: WikiResult[String]) =>
      ...
      if errs.nonEmpty then
        ...
        ((
          a.extractUnsafe,
          b.extractUnsafe
        ) == obtained.extractUnsafe) :| "Result values are not zipped correctly"
    }
  }
      ...
      if a.failure.nonEmpty || b.failure.nonEmpty then
        ...
        if errs.nonEmpty then
          ...
          ((
            a.extractUnsafe,
            b.extractUnsafe
          ) == obtained.extractUnsafe) :| "Results are not accumulated correctly"
    }
  }
    ...
    forAll(wikiResultGen[Int](true, true)) { (w: WikiResult[Int]) =>
      ...
      if w.failure.isDefined then
        sameErrorMessage(
          w.failure,
          obtained.failure
        ) :| "the old failure is not reported"
      else if w.errors.nonEmpty then
        ...
        sameErrorMessage(
          obtained.failure,
          Some(ex)
        ) :| "the new failure is not reported"
    }
  }
        ...
        if links.failure.nonEmpty || links.errors.nonEmpty then
          ...
          if nameFail.nonEmpty then
            sameErrorMessage(
              nameFail,
              result.failure
            ) :| "did not report the failure issued by nameOfArticle"
          else
            ...
            if nameErrors.nonEmpty then
              ...
              (result.extractUnsafe == results.map(
                _.extractUnsafe
              )) :| "did not construct the correct set of names"
      }
    }
    val res: Prop =
      setUp(false, false) &&
