...
trait DecoderInstances:
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction { case Json.Arr(as) =>
      as.foldLeft[Option[List[A]]](Option(List.empty)) { (acc, elem) =>
        for
          decodedList <- acc
          decodedElem <- decoder.decode(elem)
        yield decodedList :+ decodedElem
      }
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction { case Json.Obj(o) =>
      decoder.decode(o(name))
    }
